...
const actions = {
  get_city({ commit, dispatch, getters }, query) {
    return axios.get(`${zomatoUrl}/cities?q=${query}`, axiosConfig).then(async(response) => {
      ...
      if (cities.length) {
        commit('set_city', { id: cities[0].id, name: cities[0].name })
  ...
  async get_next_restaurants({ commit, dispatch, getters, rootGetters }) {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      const searchUrl = `${zomatoUrl}/search?entity_id=${getters.currentCityId}&entity_type=city&category=${category}&cuisines=${cuisine}`
